...
#define INDEX_IMAGE_IN			0
#define INDEX_IMAGE_OUT			1
#define INDEX_IMAGE_DISABLED	2
#define INDEX_IMAGE_PUSHED  	3
...
CButtonST::CButtonST()
{
	...
	SetTextPosition(ST_ALIGN_TEXT_CENTER, ST_ALIGN_TEXT_MIDDLE, { 0, 0 }, FALSE);
...
void CButtonST::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		...
		if (m_csIcons[INDEX_IMAGE_IN].hIcon)	::DestroyIcon(m_csIcons[INDEX_IMAGE_IN].hIcon);
		if (m_csIcons[INDEX_IMAGE_OUT].hIcon)	::DestroyIcon(m_csIcons[INDEX_IMAGE_OUT].hIcon);
		if (m_csIcons[INDEX_IMAGE_DISABLED].hIcon)	::DestroyIcon(m_csIcons[INDEX_IMAGE_DISABLED].hIcon);
		if (m_csIcons[INDEX_IMAGE_PUSHED].hIcon)	::DestroyIcon(m_csIcons[INDEX_IMAGE_PUSHED].hIcon);

		// Destroy bitmaps
		if (m_csBitmaps[INDEX_IMAGE_IN].hBitmap)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_IN].hBitmap);
		if (m_csBitmaps[INDEX_IMAGE_OUT].hBitmap)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_OUT].hBitmap);
		if (m_csBitmaps[INDEX_IMAGE_DISABLED].hBitmap)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_DISABLED].hBitmap);
		if (m_csBitmaps[INDEX_IMAGE_PUSHED].hBitmap)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_PUSHED].hBitmap);

		// Destroy mask bitmaps
		if (m_csBitmaps[INDEX_IMAGE_IN].hMask)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_IN].hMask);
		if (m_csBitmaps[INDEX_IMAGE_OUT].hMask)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_OUT].hMask);
		if (m_csBitmaps[INDEX_IMAGE_DISABLED].hMask)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_DISABLED].hMask);
		if (m_csBitmaps[INDEX_IMAGE_PUSHED].hMask)	::DeleteObject(m_csBitmaps[INDEX_IMAGE_PUSHED].hMask);
	} // if
...
void CButtonST::PreSubclassWindow() 
{
	...
	if ((nBS & BS_CHECKBOX) == BS_CHECKBOX || (nBS & BS_AUTORADIOBUTTON) == BS_AUTORADIOBUTTON || (nBS & BS_RADIOBUTTON) == BS_RADIOBUTTON)
	{
		m_bIsCheckBox = TRUE;
		if ((nBS & BS_AUTORADIOBUTTON) == BS_AUTORADIOBUTTON)
			m_bIsAutoRadio = TRUE;
	}
...
void CButtonST::UncheckRadioButtonsInGroup()
{
	...
	{
		if (pCtl->SendMessage(WM_GETDLGCODE, 0, 0L) & DLGC_RADIOBUTTON)
		{
			if ((pCtl->GetStyle() & WS_TABSTOP) == WS_TABSTOP)
			{
				pCtl->ModifyStyle(WS_TABSTOP, 0);
				bResetTabStop = TRUE;
			}

			if (pCtl->SendMessage(BM_GETCHECK, 0, 0L) != 0)
				pCtl->SendMessage(BM_SETCHECK, 0, 0L);
		}
	}
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_bIsCheckBox)
	{
		m_bIsPressed  =  (lpDIS->itemState & ODS_SELECTED) || (GetCheck() != 0);
	} // if
...
{
	BYTE byIndex = INDEX_IMAGE_IN;

	// Select the icon to use
	if ((m_bIsCheckBox && bIsPressed) || (!m_bIsCheckBox && (bIsPressed || m_bMouseOnButton)))
		byIndex = INDEX_IMAGE_IN;
	else
		byIndex = (m_csIcons[INDEX_IMAGE_OUT].hIcon == NULL ? INDEX_IMAGE_IN : INDEX_IMAGE_OUT);

	if (bIsPressed && m_csBitmaps[INDEX_IMAGE_PUSHED].hBitmap != NULL)
	{
		byIndex = INDEX_IMAGE_PUSHED;
		bIsPressed = FALSE;
	}

	if (bIsDisabled && m_csBitmaps[INDEX_IMAGE_DISABLED].hBitmap != NULL)
	{
		byIndex = INDEX_IMAGE_DISABLED;
		bIsDisabled = FALSE;
	}
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if ((m_bIsCheckBox && bIsPressed) || (!m_bIsCheckBox && (bIsPressed || m_bMouseOnButton)))
		...
		byIndex = (m_csBitmaps[INDEX_IMAGE_OUT].hBitmap == NULL ? INDEX_IMAGE_IN : INDEX_IMAGE_OUT);
	...
	if (bIsDisabled && m_bShowDisabledBitmap && m_csBitmaps[INDEX_IMAGE_DISABLED].hBitmap != NULL)
	{
		byIndex = INDEX_IMAGE_DISABLED;
		bIsDisabled = FALSE;
	}
...
//	pDC->FrameRect(rpCaption, &brBtnShadow);

	// Center text
	CRect centerRect = rpCaption;
	pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | DT_CENTER | DT_CALCRECT);
	
	POINT ptTextOffset = m_ptTextOffset;
	switch (m_byHorizontalAlign)
	{
	case ST_ALIGN_TEXT_CENTER:
		ptTextOffset.x += (centerRect.Width() - rpCaption->Width()) / 2;
		break;
	case ST_ALIGN_TEXT_RIGHT:
		ptTextOffset.x += (centerRect.Width() - rpCaption->Width());
		break;
	case ST_ALIGN_TEXT_LEFT:
	default:
		break;
	}

	switch (m_byVerticalAlign)
	{
	case ST_ALIGN_TEXT_MIDDLE:
		ptTextOffset.y += (centerRect.Height() - rpCaption->Height()) / 2;
		break;
	case ST_ALIGN_TEXT_BOTTOM:
		ptTextOffset.y += (centerRect.Height() - rpCaption->Height());
		break;
	case ST_ALIGN_TEXT_TOP:
	default:
		break;
	}

	rpCaption->OffsetRect(ptTextOffset.x, ptTextOffset.y);
	/* RFU
	...
	*/
	...
	pDC->SetBkMode(TRANSPARENT);
	/*
	pDC->DrawState(rCaption->TopLeft(), rCaption->Size(), (LPCTSTR)sTitle, (bIsDisabled ? DSS_DISABLED : DSS_NORMAL), TRUE, 0, (CBrush*)NULL);
	*/
	if (bIsDisabled)
	{
		...
		if (csGrayII.hbmColor)
		{
			...
			for (dwLoopY = 0; dwLoopY < dwHeight; dwLoopY++)
			{
				for (dwLoopX = 0; dwLoopX < dwWidth; dwLoopX++)
				{
					...
					//double dGray = ((GetRValue(crPixel) * 0.299) + (GetGValue(crPixel) * 0.587) + (GetBValue(crPixel) * 0.114));
					//byNewPixel = (BYTE)( pow( dGray / 255.0, dGamma) * 255.0 );
...
DWORD CButtonST::SetIcon(
	...
	int nIconOut, int nCxDesiredOut, int nCyDesiredOut,
	int nIconDisabled, int nCxDesiredDisabled, int nCyDesiredDisabled,
	int nIconPushed, int nCxDesiredPushed, int nCyDesiredPushed)
{
	// Find correct resource handle
	HINSTANCE hInstResource = AfxFindResourceHandle(MAKEINTRESOURCE(nIconIn), RT_GROUP_ICON);

	// Set icon when the mouse is IN the button
	HICON hIconIn = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconIn), IMAGE_ICON, nCxDesiredIn, nCyDesiredIn, 0);
	...
	// Set icon when the button is pressed
	HICON hIconPushed = NULL;
	if (nIconPushed != NULL)
		hIconPushed = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconPushed), IMAGE_ICON, nCxDesiredPushed, nCyDesiredPushed, 0);

	return SetIcon(hIconIn, hIconOut, hIconDisabled, hIconPushed);
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(int nIconIn, int nIconOut, int nIconDisabled, int nIconPushed)
{
	return SetIcon(nIconIn, 0, 0, nIconOut, 0, 0, nIconDisabled, 0, 0, nIconPushed, 0, 0);
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut, HICON hIconDisabled, HICON hIconPushed)
{
	ICONINFO	ii;
	...
	if (hIconIn)
	{
		...
		m_csIcons[INDEX_IMAGE_IN].hIcon = hIconIn;
		// Get icon dimension
		::ZeroMemory(&ii, sizeof(ICONINFO));
		if (::GetIconInfo(hIconIn, &ii) == FALSE)
		{
			FreeResources();
			return BTNST_INVALIDRESOURCE;
		} // if

		m_csIcons[INDEX_IMAGE_IN].dwWidth	= (DWORD)(ii.xHotspot * 2);
		m_csIcons[INDEX_IMAGE_IN].dwHeight	= (DWORD)(ii.yHotspot * 2);
		...
		if (hIconOut)
		{
			...
			m_csIcons[INDEX_IMAGE_OUT].hIcon = hIconOut;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			if (::GetIconInfo(hIconOut, &ii) == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			} // if

			m_csIcons[INDEX_IMAGE_OUT].dwWidth	= (DWORD)(ii.xHotspot * 2);
			m_csIcons[INDEX_IMAGE_OUT].dwHeight	= (DWORD)(ii.yHotspot * 2);
			::DeleteObject(ii.hbmMask);
			::DeleteObject(ii.hbmColor);
		} // if
		...
		if (hIconDisabled)
		{
			...
			m_csIcons[INDEX_IMAGE_DISABLED].hIcon = hIconDisabled;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			if (::GetIconInfo(hIconDisabled, &ii) == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			} // if

			m_csIcons[INDEX_IMAGE_DISABLED].dwWidth = (DWORD)(ii.xHotspot * 2);
			m_csIcons[INDEX_IMAGE_DISABLED].dwHeight = (DWORD)(ii.yHotspot * 2);
			::DeleteObject(ii.hbmMask);
			::DeleteObject(ii.hbmColor);
		} // if
		...
		if (hIconPushed)
		{
			m_csIcons[INDEX_IMAGE_PUSHED].hIcon = hIconPushed;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			if (::GetIconInfo(hIconPushed, &ii) == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			} // if

			m_csIcons[INDEX_IMAGE_PUSHED].dwWidth = (DWORD)(ii.xHotspot * 2);
			m_csIcons[INDEX_IMAGE_PUSHED].dwHeight = (DWORD)(ii.yHotspot * 2);
			::DeleteObject(ii.hbmMask);
			::DeleteObject(ii.hbmColor);
		} // if
	} // if
...
DWORD CButtonST::LoadBitmaps(
	UINT nIDBitmapResource,
	UINT nIDBitmapResourceSel,
	UINT nIDBitmapResourceFocus,
	UINT nIDBitmapResourceDisabled)
{
	return SetBitmaps(nIDBitmapResourceFocus, 0, nIDBitmapResource, 0, nIDBitmapResourceDisabled, 0, nIDBitmapResourceSel, 0);
}
...
//
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, HBITMAP hBitmapOut, HBITMAP hBitmapDisabled, HBITMAP hBitmapPushed)
{
	return SetBitmaps(hBitmapIn, 0, hBitmapOut, 0, hBitmapDisabled, 0, hBitmapPushed, 0);
}

DWORD CButtonST::SetBitmaps(
	int nBitmapIn, COLORREF crTransColorIn, 
	int nBitmapOut, COLORREF crTransColorOut, 
	int nBitmapDisabled, COLORREF crTransColorDisabled,
	int nBitmapPushed, COLORREF crTransColorPushed)
{
	// Find correct resource handle
	...
	// Load bitmap Disabled
	HBITMAP	hBitmapDisabled = NULL;
	switch (nBitmapDisabled)
	{
	case NULL:
		break;
	case (int)BTNST_AUTO_GRAY:
		hBitmapDisabled = (HBITMAP)BTNST_AUTO_GRAY;
		break;
	case (int)BTNST_AUTO_DARKER:
		hBitmapDisabled = (HBITMAP)BTNST_AUTO_DARKER;
		break;
	default:
		hBitmapDisabled = (HBITMAP)::LoadImage(hInstResource, MAKEINTRESOURCE(nBitmapDisabled), IMAGE_BITMAP, 0, 0, 0);
		break;
	}

	// Load bitmap Pushed
	HBITMAP	hBitmapPushed = NULL;
	if (nBitmapPushed != NULL)
		hBitmapPushed = (HBITMAP)::LoadImage(hInstResource, MAKEINTRESOURCE(nBitmapPushed), IMAGE_BITMAP, 0, 0, 0);

	return SetBitmaps(hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut, hBitmapDisabled, crTransColorDisabled, hBitmapPushed, crTransColorPushed);
} // End of SetBitmaps
...
DWORD CButtonST::SetBitmaps(
	...
	HBITMAP hBitmapOut, COLORREF crTransColorOut, 
	HBITMAP hBitmapDisabled, COLORREF crTransColorDisabled,
	HBITMAP hBitmapPushed, COLORREF crTransColorPushed)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		m_csBitmaps[INDEX_IMAGE_IN].hBitmap = hBitmapIn;
		m_csBitmaps[INDEX_IMAGE_IN].crTransparent = crTransColorIn;
		// Get bitmap size
		...
		m_csBitmaps[INDEX_IMAGE_IN].dwWidth = (DWORD)csBitmapSize.bmWidth;
		m_csBitmaps[INDEX_IMAGE_IN].dwHeight = (DWORD)csBitmapSize.bmHeight;

		// Create grayscale/darker bitmap BEFORE mask (of hBitmapIn)
		switch ((int)hBitmapOut)
		{
			case (int)BTNST_AUTO_GRAY:
				hBitmapOut = CreateGrayscaleBitmap(hBitmapIn, m_csBitmaps[INDEX_IMAGE_IN].dwWidth, m_csBitmaps[INDEX_IMAGE_IN].dwHeight, crTransColorIn);
				m_csBitmaps[INDEX_IMAGE_OUT].hBitmap = hBitmapOut;
				crTransColorOut = crTransColorIn;
				break;
			case (int)BTNST_AUTO_DARKER:
				hBitmapOut = CreateDarkerBitmap(hBitmapIn, m_csBitmaps[INDEX_IMAGE_IN].dwWidth, m_csBitmaps[INDEX_IMAGE_IN].dwHeight, crTransColorIn);
				m_csBitmaps[INDEX_IMAGE_OUT].hBitmap = hBitmapOut;
				crTransColorOut = crTransColorIn;
				break;
		} // switch
		...
		m_csBitmaps[INDEX_IMAGE_IN].hMask = CreateBitmapMask(hBitmapIn, m_csBitmaps[INDEX_IMAGE_IN].dwWidth, m_csBitmaps[INDEX_IMAGE_IN].dwHeight, crTransColorIn);
		if (m_csBitmaps[INDEX_IMAGE_IN].hMask == NULL)
		{
			FreeResources();
		...
		{
			m_csBitmaps[INDEX_IMAGE_OUT].hBitmap = hBitmapOut;
			m_csBitmaps[INDEX_IMAGE_OUT].crTransparent = crTransColorOut;
			// Get bitmap size
			...
			m_csBitmaps[INDEX_IMAGE_OUT].dwWidth = (DWORD)csBitmapSize.bmWidth;
			m_csBitmaps[INDEX_IMAGE_OUT].dwHeight = (DWORD)csBitmapSize.bmHeight;

			// Create mask for bitmap Out
			m_csBitmaps[INDEX_IMAGE_OUT].hMask = CreateBitmapMask(hBitmapOut, m_csBitmaps[INDEX_IMAGE_OUT].dwWidth, m_csBitmaps[INDEX_IMAGE_OUT].dwHeight, crTransColorOut);
			if (m_csBitmaps[INDEX_IMAGE_OUT].hMask == NULL)
			{
				FreeResources();
				return BTNST_FAILEDMASK;
			} // if
		...
		if (hBitmapDisabled)
		{
			m_csBitmaps[INDEX_IMAGE_DISABLED].hBitmap = hBitmapDisabled;
			m_csBitmaps[INDEX_IMAGE_DISABLED].crTransparent = crTransColorDisabled;
			// Get bitmap size
			nRetValue = ::GetObject(hBitmapDisabled, sizeof(csBitmapSize), &csBitmapSize);
			if (nRetValue == 0)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			} // if
			m_csBitmaps[INDEX_IMAGE_DISABLED].dwWidth = (DWORD)csBitmapSize.bmWidth;
			m_csBitmaps[INDEX_IMAGE_DISABLED].dwHeight = (DWORD)csBitmapSize.bmHeight;

			// Create mask for bitmap Disabled
			m_csBitmaps[INDEX_IMAGE_DISABLED].hMask = CreateBitmapMask(hBitmapDisabled, m_csBitmaps[INDEX_IMAGE_DISABLED].dwWidth, m_csBitmaps[INDEX_IMAGE_DISABLED].dwHeight, crTransColorDisabled);
			if (m_csBitmaps[INDEX_IMAGE_DISABLED].hMask == NULL)
			{
				FreeResources();
				return BTNST_FAILEDMASK;
			} // if

			ShowDisabledBitmap(TRUE);
		} // if

		if (hBitmapPushed)
		{
			m_csBitmaps[INDEX_IMAGE_PUSHED].hBitmap = hBitmapPushed;
			m_csBitmaps[INDEX_IMAGE_PUSHED].crTransparent = crTransColorPushed;
			// Get bitmap size
			nRetValue = ::GetObject(hBitmapPushed, sizeof(csBitmapSize), &csBitmapSize);
			if (nRetValue == 0)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			} // if
			m_csBitmaps[INDEX_IMAGE_PUSHED].dwWidth = (DWORD)csBitmapSize.bmWidth;
			m_csBitmaps[INDEX_IMAGE_PUSHED].dwHeight = (DWORD)csBitmapSize.bmHeight;

			// Create mask for bitmap Disabled
			m_csBitmaps[INDEX_IMAGE_PUSHED].hMask = CreateBitmapMask(hBitmapPushed, m_csBitmaps[INDEX_IMAGE_PUSHED].dwWidth, m_csBitmaps[INDEX_IMAGE_PUSHED].dwHeight, crTransColorPushed);
			if (m_csBitmaps[INDEX_IMAGE_PUSHED].hMask == NULL)
			{
				FreeResources();
				return BTNST_FAILEDMASK;
			} // if
...
DWORD CButtonST::SetTextPosition(BYTE byHorizontalAlign, BYTE byVerticalAlign, POINT ptTextOffset, BOOL bRepaint)
{
	switch (byHorizontalAlign)
	{
	case ST_ALIGN_TEXT_CENTER:
	case ST_ALIGN_TEXT_LEFT:
	case ST_ALIGN_TEXT_RIGHT:
		m_byHorizontalAlign = byHorizontalAlign;
		break;
	default:
		return BTNST_INVALIDALIGN;
	} // switch

	switch (byVerticalAlign)
	{
	case ST_ALIGN_TEXT_MIDDLE:
	case ST_ALIGN_TEXT_TOP:
	case ST_ALIGN_TEXT_BOTTOM:
		m_byVerticalAlign = byVerticalAlign;
		break;
	default:
		return BTNST_INVALIDALIGN;
	} // switch
...
DWORD CButtonST::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
	...
	return SetPressedOffset(ptPressedOffset, bRepaint);
} // End of SetPressedStyle

DWORD CButtonST::SetPressedOffset(POINT ptPressedOffset, BOOL bRepaint)
{
	m_ptPressedOffset = ptPressedOffset;
	if (bRepaint)
		Invalidate();
...
{
	if (byColorIndex == BTNST_COLOR_BK)
	{
		SetColor(BTNST_COLOR_BK_IN, crColor);
		SetColor(BTNST_COLOR_BK_OUT, crColor);
		return SetColor(BTNST_COLOR_BK_FOCUS, crColor, bRepaint);
	}

	if (byColorIndex == BTNST_COLOR_FG)
	{
		SetColor(BTNST_COLOR_FG_IN, crColor);
		SetColor(BTNST_COLOR_FG_OUT, crColor);
...
//			Function executed successfully.
//
DWORD CButtonST::ShowDisabledBitmap(BOOL bShowDisabledBitmap, BOOL bRepaint)
{
	m_bShowDisabledBitmap = bShowDisabledBitmap;
	// Repaint the button
	if (bRepaint) Invalidate();

	return BTNST_OK;
} // End of ShowDisabledBitmap
...
void CButtonST::SizeToContent()
{
	if (m_csIcons[INDEX_IMAGE_IN].hIcon)
	{
		m_ptImageOrg.x = 0;
		m_ptImageOrg.y = 0;
	    SetWindowPos(	NULL, -1, -1, m_csIcons[INDEX_IMAGE_IN].dwWidth, m_csIcons[INDEX_IMAGE_IN].dwHeight,
	...
	if (m_csBitmaps[INDEX_IMAGE_IN].hBitmap)
	{
		m_ptImageOrg.x = 0;
		m_ptImageOrg.y = 0;
	    SetWindowPos(	NULL, -1, -1, m_csBitmaps[INDEX_IMAGE_IN].dwWidth, m_csBitmaps[INDEX_IMAGE_IN].dwHeight,
						SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOACTIVATE);
	} // if
